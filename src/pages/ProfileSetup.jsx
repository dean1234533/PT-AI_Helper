...
export default function ProfileSetup() {
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 bg-violet-600 text-white rounded-md text-xs font-semibold"
              >
                Analyze Body
              </button>
              <button
  ...
  const content = (
    <div className="min-h-screen bg-slate-950 text-white pb-16 relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/4 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-violet-600/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-12">
        ...
        <div className="mb-10">
          <div className="flex justify-between items-center relative">
            ...
            <div
              className="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-gradient-to-r from-violet-600 to-emerald-500 z-0 transition-all duration-300"
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 border ${
                      isCompleted
                        ? 'bg-gradient-to-tr from-violet-600 to-emerald-500 border-transparent text-white'
                        : isActive
                  ...
                  <span
                    className={`hidden sm:block text-xs font-medium mt-2 transition-all ${
                      isActive ? 'text-violet-400 font-bold' : isCompleted ? 'text-slate-300' : 'text-slate-500'
                    }`}
                  >
                    {step.title}
                  </span>
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 sm:p-10 backdrop-blur-xl shadow-2xl relative">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950/80 border border-slate-800 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 rounded-2xl py-3 px-4 text-slate-100 text-sm outline-none transition-all"
                  />
                </div>
                    ...
                    {['Male', 'Female', 'Other'].map((g) => (
                      <button
                        ...
                        className={`py-3 px-4 border text-sm font-semibold rounded-2xl transition-all ${
                          formData.gender === g
                            ? 'bg-gradient-to-tr from-violet-600 to-violet-700 border-transparent text-white'
                    ...
                    <button
                      ...
                      className={`p-4 border text-left rounded-2xl transition-all flex flex-col gap-1 ${
                        formData.goal === item.val
                          ? 'bg-gradient-to-tr from-violet-600 to-violet-700 border-transparent text-white ring-2 ring-violet-400/35'
                          : 'bg-slate-950/40 border-slate-850 hover:border-slate-700 text-slate-400 hover:text-slate-200'
                      }`}
                    >
                      <span className="font-semibold text-sm">{item.val}</span>
                      <span className={`text-xs ${formData.goal === item.val ? 'text-violet-200' : 'text-slate-500'}`}>
                        {item.desc}
                      </span>
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950/80 border border-slate-800 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 rounded-2xl py-3.5 px-4 text-slate-100 text-sm outline-none transition-all cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    >
                      ...
                      <span className={`text-[10px] text-center ${formData.fitnessLevel === level.value ? 'text-violet-200' : 'text-slate-500'}`}>
                        {level.desc}
                      </span>
                    </button>
                  ))}
          ...
          {currentStep === 4 && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-800 hover:border-violet-500/50 rounded-3xl p-8 bg-slate-950/20 transition-all relative">
                {imagePreview ? (
                  <div className="relative group max-w-xs w-full rounded-2xl overflow-hidden shadow-2xl">
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                      <label className="p-3 bg-violet-600 hover:bg-violet-500 rounded-full cursor-pointer text-white shadow-lg transition-transform hover:scale-105">
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-64 cursor-pointer group">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center space-y-3">
                      <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl text-slate-400 group-hover:text-violet-400 group-hover:border-violet-500/30 group-hover:shadow-lg group-hover:shadow-violet-500/5 transition-all">
          ...
          <div className="flex justify-between items-center mt-10 pt-6 border-t border-slate-800/80">
            ...
            {currentStep < STEPS.length - 1 ? (
              <button
                onClick={handleNext}
                className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-500 hover:to-violet-600 text-white text-sm font-semibold rounded-2xl shadow-lg shadow-violet-600/10 transition-all hover:shadow-violet-600/20"
              >
                Next Step
