/**
 * POST /api/send-plan
 * Sends a fitness plan to a client via email.
...
function buildEmailHtml({ clientName, trainerNotes, plan }) {
  ...
  const keyRulesHtml = n?.keyRules?.length
    ? n.keyRules.map((r) => `<li style="margin-bottom:8px;font-size:14px;color:#374151">${r}</li>`).join('')
    : '';
...
export async function onRequestPost(ctx) {
  const env = ctx.env;
  try {
    const { clientEmail, clientName, trainerNotes, plan } = await ctx.request.json();

    if (!clientEmail || !plan) {
      return Response.json({ error: 'clientEmail and plan are required' }, { status: 400, headers: CORS });
    }
