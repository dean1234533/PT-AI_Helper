...
export async function onRequestPost(ctx) {
  const env = ctx.env;
  try {
    const { clientEmail, clientName, trainerName, videoCallLink, callNotes } = await ctx.request.json();
    if (!clientEmail || !videoCallLink) {
      return Response.json({ error: 'clientEmail and videoCallLink are required' }, { status: 400, headers: CORS });
    }
